...
def split_class_dir_fixed(class_dir, output, fixed, seed):
    ...
    split_valid = split_train + fixed[0]

    li = split_files(files, split_train, split_valid, len(fixed) == 2)
...
def split_class_dir_ratio(class_dir, output, ratio, seed):
    ...
    split_valid = split_train + int(ratio[1] * len(files))

    li = split_files(files, split_train, split_valid, len(ratio) == 3)
    copy_files(li, class_dir, output)


def split_files(files, split_train, split_valid, use_test):
    """Splits the files along the provided indices
    """
    files_train = files[:split_train]
    files_valid = files[split_train:split_valid] if use_test else files[split_train:]

    li = [(files_train, 'train'), (files_valid, 'valid')]

    # optional test folder
    if use_test:
        files_test = files[split_valid:]
